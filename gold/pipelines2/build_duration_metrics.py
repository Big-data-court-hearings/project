...
def compute_stats(con: duckdb.DuckDBPyConnection, group_col: str, output_path: str):
    ...
    COPY (
        ...
        WHERE duration_days IS NOT NULL 
          AND year_quarter_filed IS NOT NULL
          AND duration_days >= 0
          AND CAST(SUBSTR(year_quarter_filed, 1, 4) AS INTEGER) > {START_YEAR}
        GROUP BY year_quarter_filed, {group_col}
        ORDER BY year_quarter_filed, {group_col}
